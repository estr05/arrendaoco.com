...
class PDF extends FPDF {
    ...
    function FancyTable($header, $data) {
        ...
        foreach ($data as $row) {
            $this->Cell($w[0], 6, utf8_decode($row['id_user']), 'LR', 0, 'C', $fill);
            $this->Cell($w[1], 6, utf8_decode($row['user']), 'LR', 0, 'C', $fill);
            $this->Cell($w[2], 6, utf8_decode($row['pass']), 'LR', 0, 'C', $fill);
            $this->Cell($w[3], 6, utf8_decode($row['nombre']), 'LR', 0, 'C', $fill);
            $this->Cell($w[4], 6, utf8_decode($row['direccion']), 'LR', 0, 'C', $fill);
            $this->Cell($w[5], 6, utf8_decode($row['tel']), 'LR', 0, 'C', $fill);
            $this->Cell($w[6], 6, utf8_decode($row['correo']), 'LR', 0, 'C', $fill);
            $this->Ln();
            $fill = !$fill;
        }
...
$consulta = "SELECT id_user, user, pass, nombre, direccion, tel, correo FROM usuarios";
...
$header = array('ID', 'Usuario', 'Contraseña', 'Nombre', 'Dirección', 'Teléfono', 'Correo');
